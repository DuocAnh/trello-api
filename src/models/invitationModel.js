...
const findOneById = async (invitationId) => {
  try {
    const result = await GET_DB().collection(INVITATION_COLLECTION_NAME).findOne({ _id: new ObjectId
    (invitationId) })
    return result
  } catch (error) { throw new Error(error) }
}
...
const findByUser = async (userId) => {
  try {
    ...
    const results = await GET_DB().collection(INVITATION_COLLECTION_NAME).aggregate([
      ...
      { $lookup: {
        from: userModel.USER_COLLECTION_NAME, // người đi mời
      ...
      { $lookup: {
        from: userModel.USER_COLLECTION_NAME, // người được mời
      ...
      { $lookup: {
        from: boardModel.BOARD_COLLECTION_NAME, // thông tin của board
